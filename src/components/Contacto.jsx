...
function Contacto() {
  ...
  const [modoEdicion, setModoEdicion] = useState(null) // id de la reserva en edición
  const [mostrarLista, setMostrarLista] = useState(false)

  // ── LOCAL STORAGE — CRUD ───────────────────────────────────────────────
  // READ: cargar reservas al montar el componente
    ...
    <section id="contacto" style={{ background: "var(--crema)", padding: "6rem 6%" }}>
      <div style={{
        ...
        <div>
          ...
          {[
            { icono: "📍", label: "Dirección", texto: "Av. Providencia 2250, Local 3 — Providencia, Santiago" },
            { icono: "🕐", label: "Horario",   texto: "Lunes a viernes: 9:00 – 20:00 / Sábado: 9:00 – 17:00" },
            { icono: "📱", label: "WhatsApp",  texto: "+56 9 8765 4321" },
            { icono: "📧", label: "Email",     texto: "hola@serenaspa.cl" }
          ].map((d, i) => (
            <div key={i} style={{ display: "flex", gap: "1rem", marginBottom: "1.3rem", alignItems: "flex-start" }}>
              <span style={{ fontSize: "1rem", flexShrink: 0, marginTop: "2px" }}>{d.icono}</span>
              <div>
                <strong style={{
                  display: "block", fontSize: "0.68rem", fontWeight: 500,
                  letterSpacing: "0.12em", textTransform: "uppercase",
